# cloud-track-emulator-runtime Specification

## Purpose

The zero-cost local AWS substrate for Track 05's emulator-dependent labs: [floci](https://github.com/floci-io/floci)
as the pinned default `make up` backend (replacing LocalStack, whose community edition sunset in March
2026), the requirement that labs stay endpoint-agnostic so a real-AWS route is one variable away,
full removal of LocalStack, per-lab coverage verification with honesty caveats where the emulator
can't carry a lesson, and the Track 05 prose that frames the migration.
