# aiops-track-oss500-curriculum Specification

## Purpose
TBD - created by archiving change add-aiops-oss500-edition. Update Purpose after archive.
## Requirements
### Requirement: Track 12 rebuilt in place in the OSS-500 methodology
The change SHALL rebuild Track 12 in place: the 11 module `README.md` files at
`tracks/12-ai-augmented-ops/modules/<NN>/` and the overview at `tracks/12-ai-augmented-ops/README.md`,
authored to the OSS-500 methodology, such that `mkdocs build --strict` passes and Track 12 reads as the
canonical track (no staging directory, no "edition/experimental" framing). Existing per-module
`cheatsheet.md` files SHALL be kept.

#### Scenario: Track builds and reads as canonical
- **WHEN** `mkdocs build --strict` is run
- **THEN** it exits 0 with no orphan-page or broken-link error, and Track 12 renders the rebuilt content
...
Each module SHALL be anchored to a real AI-security incident, finding, or authoritative framework
(e.g. OWASP LLM Top-10, MITRE ATLAS), with the mechanism taught in our own prose and the attack/finding
narrative linked to a resolving authoritative source. The change SHALL NOT invent URLs; an uncertain
new link carries `<!-- VALIDATE -->` and is resolved before ship.
...
authored to `VISUAL-CONVENTIONS.md` (type follows concept, placed atop the section it summarizes, no
inline colors), and SHALL use de-densifying formatting (callouts, tables, "at a glance" leads) to break
up walls of text.
...
preserving every real link, ID, and gotcha from the source module.
