...
Every entry is a real, public repo with a committed deliverable you can clone and run, so each link
...
2. **Self-check against the lab's Success criteria** and commit the deliverable — honor system, the
   committed work is what backs your entry.
...
4. **Submit it** one of two ways:
    ...
    - Open a PR adding a row to the table above (handle, track, one-line description, repo link).
      A maintainer reviews for the hygiene checklist and merges.
...
to flatter. The proof is the work itself: a public repo you can clone, run, and read the history of.
If a capstone interests you, the repo's history shows the work, and the learner is reachable in the
