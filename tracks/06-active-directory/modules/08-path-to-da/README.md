...
!!! abstract "In 60 seconds"
    This is the phase project: chain modules 02–07 into one walked `jsmith → Domain Admins` path on
    the Corp BloodHound dataset. The hard part isn't running BloodHound — it's reading the graph and
    *writing the narrative*: which path a real attacker would actually take (not just the shortest),
    each hop mapped to its ATT&CK technique and the artefact it leaves, and the hop-by-hop mitigation
    priority. The deliverable is a client-actionable attack-path report, and the skill is synthesis,
    not execution.
...
!!! note "The mental model"
    The graph is a road map and **each edge is a road with its own toll, speed limit, and traffic
    camera** — BloodHound draws them all the same width, but they are not equal. `MemberOf` is free;
    `GenericWrite` is one LDAP write; `AdminTo` is PTH or Kerberos; `HasSession` is a cached
    privileged credential on a reachable host (often the most valuable edge). You're not finding the
    *shortest* path, you're choosing the one that minimises noise and maximises reach.
...
!!! warning "The gotcha"
    ...
    path and confirm preconditions hold *on the day*; a report that asserts a path you never verified
    is a finding waiting to be wrong in front of the client.
...
!!! tip "AI caveat"
    A model writes a clean per-hop client narrative from the path JSON — one paragraph per hop, no
    jargon — and does it well. Your job is the fact-check: does it name the technique, the generated
    (or absent) event, and the *correct* mitigation for each hop? Verify each claim against that
    technique's ATT&CK page before the narrative ships.
...
!!! question "Check yourself"
    - Why is the *shortest* path through the graph often not the path a real attacker takes?
    - Why is a BloodHound edge a hypothesis rather than a guarantee — and which edge type is most likely to have gone stale by the time you walk it?
    - When you prioritise mitigations for the client, what makes one fix higher-priority than another?
