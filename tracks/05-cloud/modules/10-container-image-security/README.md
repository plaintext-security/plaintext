...
## Objective
Scan real container images and a deliberately misconfigured `Dockerfile` with `trivy` and `grype`, interpret severity and fixability, then *harden* the Dockerfile (digest-pinned base, no secrets in `ENV`, non-root `USER`, lean install) and re-scan to prove the MEDIUM/HIGH findings are gone — and produce an image-hygiene report that gates a pull request before a flawed image reaches the registry.
