...
{track:"00 · foundations", id:"fnd-04-linux", title:"Linux for Security",
 engine:"passwd/log parsing (JS)",
 capstone:"Audit a Linux box: users, SUID binaries, cron, and listening services — write up what you’d harden.",
 steps:[
  {type:"mcq", t:"Bite 1 / permissions", q:"You generate an SSH key. Why `chmod 600 id_rsa`?",
   options:[{t:"To make it executable"},{t:"So only the owner can read/write it — SSH refuses world-readable keys",correct:true},{t:"To encrypt the key"}], explain:"600 = owner rw, nothing for group/other. Private keys must not be readable by others."},
  {type:"run", t:"Bite 2 / find the backdoor account", prose:"/etc/passwd lines. A backdoor adds a second UID-0 (root) account. Click run to spot it.",
   code:"root:x:0:0:root:/root:/bin/bash\nwww:x:33:33:www:/var/www:/usr/sbin/nologin\nsupport:x:0:0::/home/support:/bin/bash",
   run:async(code)=>{ const bad=code.split("\n").map(l=>l.split(":")).filter(p=>p[2]==="0"&&p[0]!=="root").map(p=>p[0]);
     return {output:bad.length?("UID-0 non-root accounts: "+bad.join(", ")):"none found", pass:bad.length>0, msg:bad.length?"`support` has UID 0 — root in disguise. The kernel trusts the UID, not the name.":"Check the third field (UID)."}; }},
  {type:"run", t:"Bite 3 / read the auth log", prose:"Click run to tally ‘Failed password’ by source IP.",
   code:"Failed password for root from 45.83.64.1\nAccepted password for bob from 10.0.0.5\nFailed password for root from 45.83.64.1\nFailed password for admin from 45.83.64.1\nFailed password for root from 203.0.113.9",
   run:async(code)=>{ const c={}; code.split("\n").forEach(l=>{const m=l.match(/Failed password.* from (\S+)/); if(m)c[m[1]]=(c[m[1]]||0)+1;}); const t=Object.entries(c).sort((a,b)=>b[1]-a[1]); const ok=t.length&&t[0][0]==="45.83.64.1";
     return {output:t.map(x=>x[0]+"  "+x[1]+" fails").join("\n"), pass:ok, msg:ok?"45.83.64.1 dominates the failures — block/rate-limit it. This tally is the seed of a real detection.":"Run it."}; }}
 ]},
