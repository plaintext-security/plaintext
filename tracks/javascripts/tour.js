...
{track:"08 · cryptography", id:"01-primitives-practice", title:"Crypto Primitives",
 ...
 steps:[
  ...
  {type:"input", t:"Idea 2 / hit a target digest (flag)", prose:"Find the word that hashes to <code>2c624232…</code> (sha256). hint: it's the literal string <code>password</code>… try variations until the prefix matches.",
   ...
   verify:async(v)=>{ const h=await sha256hex(v.trim()); const ok=h.startsWith("5e884898da280471"); return {pass:ok, out:`sha256(${v.trim()}) = ${h.slice(0,24)}…`, msg: ok?"Matched — that's how a `flag` check works: we store the hash, you prove you found the input. Zero answer-leak.":"Not it — but you can see the digest. (It's the classic weak password.)"};}},
...
const ENRICH = {"02-lab-setup": {"tiers": ["T0", "T0", "T1"], "learn": ["A security lab runs hostile software <i>on purpose</i>. <b>Isolation is control #1</b>: keep the vulnerable box on a network that reaches only itself — never your home LAN or the internet.", "Compose files encode that isolation. <code>network_mode: host</code> hands the container your real network; a named/<code>internal</code> network walls it off. You fix the environment by editing the environment.", "Bringing it up and confirming from the shell is itself the skill — and where a <b>real Linux shell</b> belongs (Tier&nbsp;1: WebVM/v86 in-browser, or your own Docker)."], "links": [{"href": "https://docs.docker.com/network/", "label": "Docker networking (docs)"}, null, null], "realenv": {"t": "Do it for real", "learn": "Spin the actual lab in a free GitHub Codespace — real Docker, real isolation — then prove it’s contained.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch lab in Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{lab-isolated}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{lab-isolated}</code>", "flagPrefix": "09987a25ca9d1a4e"}}, "08-web-ssrf-xxe": {"tiers": ["T0", "T0", "T0"], "learn": ["SSRF abuses a server feature that fetches a URL <i>for you</i>. Because the request originates inside the trust boundary, you can reach things you can’t from outside — like the cloud metadata service.", "The pattern generalises: SSRF, XXE and injection are all <b>untrusted input crossing into a privileged interpreter/requester</b>. Always ask: <i>who actually makes this request, and what can they reach?</i>", "XXE is the same move against an XML parser — an external entity makes the parser read a local file. The fix is structural: disable DTDs/external entities."], "links": [{"href": "https://portswigger.net/web-security/ssrf", "label": "PortSwigger — SSRF (free labs)"}, null, {"href": "https://owasp.org/www-community/vulnerabilities/XML_External_Entity_(XXE)_Processing", "label": "OWASP — XXE"}], "realenv": {"t": "Exploit a real SSRF CVE", "learn": "Bring up a Vulhub SSRF target in a Codespace and pull the (mock) metadata creds against a live server.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{ssrf-169254}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{ssrf-169254}</code>", "flagPrefix": "99737b77559466f8"}}, "05-intrusion-detection": {"tiers": ["T0", "T0", "T0"], "learn": ["An IDS rule fires when traffic matches a pattern. The cheap version is one content string — but cheap rules are noisy.", "Precision is the craft: add predicates until you hit the attack and <i>only</i> the attack. A rule is defined as much by what it <b>doesn’t</b> match.", "False-positive economics: a noisy rule gets muted, then misses the real thing. Tune against a benign baseline before you ship."], "links": [{"href": "https://docs.suricata.io/en/latest/rules/intro.html", "label": "Suricata — rule syntax"}, null, null], "realenv": {"t": "Run real Suricata", "learn": "Replay a PCAP through actual Suricata in a Codespace and confirm your rule fires.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{alert-fired}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{alert-fired}</code>", "flagPrefix": "ac40ed270dcbff4a"}}, "10-log-cloud-forensics": {"tiers": ["T0", "T0", "T0"], "learn": ["Cloud forensics is timeline reconstruction over API logs. CloudTrail records who did what, from where, when — your whole case is in the JSON.", "The pivot is where a trusted identity turns hostile: same user, new IP, sudden privilege. Spotting that transition is the core skill.", "Attackers cover tracks (<code>DeleteTrail</code>, <code>StopLogging</code>). Ship logs off-account so the cover-up <i>itself</i> becomes evidence."], "links": [{"href": "https://docs.aws.amazon.com/awscloudtrail/latest/userguide/cloudtrail-concepts.html", "label": "AWS — CloudTrail concepts"}, null, null], "realenv": {"t": "Query a real export", "learn": "Open a CloudTrail export in a Codespace and reconstruct the intrusion with <code>jq</code>.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{deletetrail}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{deletetrail}</code>", "flagPrefix": "8c6125f78a02bae9"}}, "13-detection-reporting": {"tiers": ["T0", "T0", "T0"], "learn": ["YARA matches malware on strings/bytes plus a condition. Start by catching the sample.", "Then the hard part: <b>don’t match goodware</b>. A unique artifact (a C2 string) beats a generic one (<code>MZ</code>, in every PE).", "Reporting turns a match into shippable intel: the IOC + the ATT&CK technique + your confidence."], "links": [{"href": "https://yara.readthedocs.io/en/stable/writingrules.html", "label": "YARA — writing rules"}, null, null], "realenv": {"t": "Compile real YARA", "learn": "Scan a MalwareBazaar sample with the real <code>yara</code> CLI in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{yara-hit}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{yara-hit}</code>", "flagPrefix": "85af614421df4429"}}, "05-posture-auditing": {"tiers": ["T0", "T0", "T0"], "learn": ["CSPM tools emit a flood of findings. The skill isn’t running the scanner — it’s <b>triage</b>: severity × exploitability × blast-radius.", "Remediation is editing the artifact. Remove the public principal and the bucket is private; a re-scan flips the finding to PASS.", "The exec summary is two numbers: how many FAIL, and the worst one."], "links": [{"href": "https://github.com/prowler-cloud/prowler", "label": "Prowler (open-source CSPM)"}, null, null], "realenv": {"t": "Run real Prowler", "learn": "Point Prowler at a CloudGoat/sandbox account in a Codespace and triage live output.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{public-bucket}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{public-bucket}</code>", "flagPrefix": "68c649605b639170"}}, "08-path-to-da": {"tiers": ["T0", "T0", "T2"], "learn": ["BloodHound models AD as a graph — users, groups, machines, and the rights between them. Attack paths are just <b>reachability</b>.", "Finding the path is the defender’s skill too: cut one edge and the route to Domain Admin disappears.", "Executing the chain (Kerberoast → crack → ACL abuse) is real tradecraft — it belongs in a live AD range (Tier&nbsp;2/3), not a browser."], "links": [{"href": "https://bloodhound.specterops.io/", "label": "BloodHound (docs)"}, null, null], "realenv": {"t": "Walk the chain live", "learn": "Stand up the GOAD-style AD range and run the path end-to-end to Domain Admin.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch AD range", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{domain-admin}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{domain-admin}</code>", "flagPrefix": "9cf3699c0ade85c4"}}, "11-host-boot-integrity": {"tiers": ["T0", "T0", "T0"], "learn": ["Measured boot hashes each boot component into a TPM PCR, chaining them: <code>PCR = H(PCR ‖ component)</code>. The final value fingerprints the whole chain.<details><summary>Go deeper: why chaining?</summary>Because each measurement folds into the previous, you can’t alter an early component without changing every later PCR — there’s nowhere to hide a swap.</details>", "Tamper one component and every later PCR diverges from the golden value — detection by cryptography, not signatures.", "It beats AV because trust is rooted in hardware and measured <i>before</i> the OS, so malware in that OS can’t forge a clean result."], "links": [{"href": "https://wiki.archlinux.org/title/Trusted_Platform_Module", "label": "Arch Wiki — TPM"}, null, null], "realenv": {"t": "Read real PCRs", "learn": "Read PCRs off a vTPM with <code>tpm2-tools</code> in a Codespace and diff against golden.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{pcr-mismatch}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{pcr-mismatch}</code>", "flagPrefix": "cc2ff99694d56323"}}, "01-primitives-practice": {"tiers": ["T0", "T0", "T0"], "learn": ["A hash is deterministic, one-way, fixed-length. Flip one input bit and ~half the output bits flip (avalanche).", "That property powers the <b>flag check</b> you keep seeing: we store a hash, you prove you found the input — zero answer-leak.", "A bare hash proves integrity, not authenticity. An HMAC adds a secret key, so only a key-holder can forge the tag."], "links": [{"href": "https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto", "label": "MDN — SubtleCrypto"}, null, null], "realenv": {"t": "Sign with OpenSSL", "learn": "Generate a key and sign/verify a file with real <code>openssl</code> in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{hmac-verified}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{hmac-verified}</code>", "flagPrefix": "112d1105a09e7637"}}, "03-structured-data-reporting": {"tiers": ["T0", "T0", "T0"], "learn": ["Security work drowns in rows — scans, logs, findings. Step one is filtering to what matters.", "A report is the few numbers a decision-maker reads: distinct exposed hosts, not raw row counts.", "Dedupe before counting — raw output double-counts hosts, and an inflated number costs you credibility."], "links": [{"href": "https://docs.python.org/3/library/csv.html", "label": "Python — csv module"}, null, null], "realenv": {"t": "Run the real Python", "learn": "Run the actual parser over an nmap CSV in a Codespace and emit a Markdown report.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{report-built}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{report-built}</code>", "flagPrefix": "fa14b55f90203152"}}, "08-soar-fundamentals": {"tiers": ["T0", "T0", "T0"], "learn": ["A SOAR playbook is a pipeline: <b>enrich → decide → act</b>. Order matters — acting before deciding is how you auto-isolate the CEO.", "The decision is a threshold on confidence. Too high misses; too low auto-contains on noise.", "Reversible actions (enrich, tag) automate freely; destructive ones (disable user, isolate host) stay human-gated."], "links": [{"href": "https://github.com/Shuffle/Shuffle", "label": "Shuffle (open-source SOAR)"}, null, null], "realenv": {"t": "Fire a real playbook", "learn": "Wire a Shuffle workflow in a Codespace and trigger it on a webhook alert.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{playbook-fired}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{playbook-fired}</code>", "flagPrefix": "498821ddadf14145"}}, "08-policy-as-code": {"tiers": ["T0", "T0", "T0"], "learn": ["Zero trust is <b>default-deny</b>: access is granted only when explicit conditions hold. You write those conditions as a policy.", "Identity alone isn’t enough — device posture is a first-class input. The missing predicate is what denies the non-compliant insider.", "As code, the policy is versioned, unit-tested, reviewed, and identical at every enforcement point.<details><summary>Go deeper: the twin</summary>This is detection-as-code’s twin — a file in git, tested in CI, deployed everywhere — applied to access instead of alerts.</details>"], "links": [{"href": "https://www.openpolicyagent.org/docs/latest/policy-language/", "label": "OPA — Rego language"}, null, null], "realenv": {"t": "Run real OPA", "learn": "Evaluate your Rego with the real <code>opa</code> CLI and its unit tests in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{policy-passes}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{policy-passes}</code>", "flagPrefix": "c548c9e02bcc4e0d"}}, "05-building-mcp-servers": {"tiers": ["T0", "T0", "T0"], "learn": ["A model can only call a tool it’s been <i>described</i>. The JSON schema is the entire contract between the model and your code.", "The MCP loop: model emits <code>tool_use</code> → your handler runs → <code>tool_result</code> goes back → model continues. Closing that loop <i>is</i> an MCP server.", "A callable tool is an attack surface: the model (steerable by untrusted content) will pass bad args. Validate, scope, never trust the call blindly."], "links": [{"href": "https://modelcontextprotocol.io/", "label": "Model Context Protocol (spec)"}, null, null], "realenv": {"t": "Run a real MCP server", "learn": "Launch a real MCP server + inspector in a Codespace and watch a client call your tool.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{tool-called}", "okMsg": "Verified — that’s the full real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab this comes from <code>make check</code>. For the POC, paste: <code>FLAG{tool-called}</code>", "flagPrefix": "4f8a7d4b73551523"}}, "fnd-01-security-principles": {"tiers": ["T0", "T0", "T0"], "learn": ["Before tools comes the shared language. The <b>CIA triad</b> is what we protect — confidentiality, integrity, availability — and nearly every attack maps to degrading one of the three.", "<b>AAA</b> is how we control access: authentication (who are you), authorization (what may you do), accounting (what did you do). Keeping authn vs authz straight prevents a huge class of bugs.", "Risk ≈ threat × vulnerability × impact. You don’t fix everything — you fix what’s both exploitable and impactful."], "links": [{"href": "https://www.cisa.gov/known-exploited-vulnerabilities-catalog", "label": "CISA — Known Exploited Vulnerabilities"}, null, null], "realenv": {"t": "Analyse a real breach", "learn": "Pull a CISA KEV entry + vendor post-mortem in a Codespace and map it to CIA/AAA + the missing control.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{cia-mapped}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{cia-mapped}</code>", "flagPrefix": "507bf71f7cad85d0"}}, "fnd-02-lab-setup": {"tiers": ["T0", "T0", "T1"], "learn": ["A security lab runs hostile software on purpose. <b>Isolation is control #1</b>: keep the box on a network that reaches only itself.", "Compose files encode that isolation. <code>network_mode: host</code> hands over your real network; a named/<code>internal</code> network walls it off.", "Bringing it up and confirming from the shell is the skill — and where a <b>real Linux shell</b> belongs (Tier 1: WebVM, or your own Docker)."], "links": [{"href": "https://docs.docker.com/network/", "label": "Docker networking"}, null, null], "realenv": {"t": "Bring up the real lab", "learn": "Spin the lab in a Codespace — real Docker, real isolation — then prove it’s contained.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch lab in Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{lab-isolated}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{lab-isolated}</code>", "flagPrefix": "09987a25ca9d1a4e"}}, "fnd-03-docker": {"tiers": ["T0", "T0", "T1"], "learn": ["An image is a frozen filesystem; a container is a running instance of it. One image → many containers.", "A compose file’s published ports are your exposed attack surface — read them before you trust an environment.", "The container lifecycle (build → run → inspect) is muscle memory you’ll use in every other track."], "links": [{"href": "https://docs.docker.com/get-started/", "label": "Docker — Get started"}, null, null], "realenv": {"t": "Run real Docker", "learn": "Build and run your own container in a Codespace; confirm it’s live.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{container-up}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{container-up}</code>", "flagPrefix": "3b5c88e16430e08d"}}, "fnd-04-linux": {"tiers": ["T0", "T0", "T0", "T0", "T0", "T0", "T0", "T1"], "learn": ["In Linux, <b>everything is a file</b>: processes, devices, even live kernel state show up under <code>/proc</code>, <code>/dev</code>, <code>/sys</code>. One set of text tools reads them all — which is exactly why text-processing is a security power tool, not busywork.", "Permissions are the whole OS access-control story in nine bits: <b>rwx</b> for user / group / other, enforced by the kernel on every open. A stray <i>world-writable</i> config or cron file is a direct foothold.", "The <b>SUID</b> bit makes a binary run as its <i>owner</i> (often root), not as you. Necessary for <code>passwd</code>; lethal for a shell-capable binary, which simply hands you the owner’s privileges. This one bit seeds half of Linux privilege escalation.", "Two paths to root: <b>be UID 0</b>, or belong to a group sudoers trusts. Auditing a host starts here — a second UID-0 account or an unexpected sudo member is how persistence hides in plain sight.", "Reading what’s <b>running</b> is triage step one. Sorting processes by CPU surfaces the runaway or crypto-miner instantly — the fastest ‘is this box owned?’ signal you have.", "The text-processing <b>pipeline</b> — <code>grep | awk | sort | uniq -c | sort -rn</code> — is what you actually live in: the one line that matters among a million. Every later track (privesc, forensics, detection) comes back to it.", "A pipeline is a sentence — each stage does one job. Read it that way (filter → extract → group → count → rank) and you can build any triage one-liner from scratch.", "Reading planted output builds pattern-recognition; driving a <b>real shell</b> builds fluency — you want both. And remember: a wrong <code>rm</code> or <code>chmod</code> on a live box has no undo, so check any generated command against <code>man</code> before you run it."], "links": [{"href": "https://man7.org/linux/man-pages/man5/proc.5.html", "label": "man 5 proc"}, {"href": "https://man7.org/linux/man-pages/man1/chmod.1.html", "label": "man 1 chmod"}, {"href": "https://gtfobins.github.io/", "label": "GTFOBins — SUID/privesc reference"}, {"href": "https://man7.org/linux/man-pages/man5/passwd.5.html", "label": "man 5 passwd"}, {"href": "https://man7.org/linux/man-pages/man1/ps.1.html", "label": "man 1 ps"}, {"href": "https://man7.org/linux/man-pages/man1/grep.1.html", "label": "man 1 grep"}, {"href": "https://man7.org/linux/man-pages/man1/uniq.1.html", "label": "man 1 uniq"}, null], "realenv": {"t": "Do it for real — the lab (local)", "learn": "The module ships a real container: Ubuntu 22.04 with planted users and a genuine SSH auth log. Run it locally and answer the triage questions by hand — by-hand first, scripted second.", "href": "../../00-foundations/modules/04-linux/lab/", "launchLabel": "Open the lab → run with make up", "afterLaunch": "<b>Setup:</b> <code>git clone</code> the labs repo, <code>cd foundations/04-linux</code>, <code>make up && make shell</code>. <b>Do:</b> (1) list local accounts + who can root, (2) find the SUID set, (3) processes by CPU, (4) rank failed logins per IP. Then paste the flag <code>make check</code> prints:", "placeholder": "FLAG{...}", "flagPrefix": "ad42e28166f4eadc", "okMsg": "That’s the model: in-browser bites build the reflexes, the <b>local lab</b> is where you do it for real and produce the artifact.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{linux-triaged}</code>"}}, "fnd-05-windows": {"tiers": ["T0", "T0", "T0"], "learn": ["Windows speaks in Event IDs: 4624 success, 4625 failure, 1102 audit-cleared. Knowing the vocabulary is half of triage.", "A burst of 4625 against one account is password-guessing; a 4624 right after is success-after-failures.", "Tiering admin credentials (everyday standard account + separate admin) limits blast radius — least privilege on the desktop."], "links": [{"href": "https://www.ultimatewindowssecurity.com/securitylog/encyclopedia/", "label": "Windows Security Log Encyclopedia"}, null, null], "realenv": {"t": "Triage real EVTX", "learn": "Run a log-triage tool over a Security.evtx sample in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{event-4624}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{event-4624}</code>", "flagPrefix": "69d1e43027db80d0"}}, "fnd-06-networking": {"tiers": ["T0", "T0", "T0"], "learn": ["A port is a <b>hypothesis</b>, not an answer — convention says 443=HTTPS, but you confirm with a banner.", "Subnetting is the math that scopes a scan and segments a network; a /24 gives 254 usable hosts.", "Well-known ports are a map: 22 SSH, 80 HTTP, 445 SMB — and exposed SMB is a frequent breach origin."], "links": [{"href": "https://www.practicalnetworking.net/series/packet-traveling/packet-traveling/", "label": "Practical Networking — Packet Traveling"}, null, null], "realenv": {"t": "Read real packets", "learn": "Capture with tcpdump and scope a subnet with nmap in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{subnet-ok}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{subnet-ok}</code>", "flagPrefix": "1269a98d47615c2b"}}, "fnd-07-web-http": {"tiers": ["T0", "T0", "T0"], "learn": ["HTTP status codes carry meaning: 401 = authenticate, 403 = authenticated-but-forbidden. 403 on an authenticated request smells like broken access control.", "A numeric object id in a URL (<code>?id=5</code>) invites IDOR: does <code>id=6</code> return someone else’s data?", "Cookies/tokens are often just base64 — readable, and tamperable unless signed. Encoding is not secrecy."], "links": [{"href": "https://developer.mozilla.org/en-US/docs/Web/HTTP", "label": "MDN — HTTP"}, null, null], "realenv": {"t": "Drive real HTTP", "learn": "curl an app in a Codespace, tamper a parameter, and observe the authz check.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{http-200}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{http-200}</code>", "flagPrefix": "eb03e6dffe9d96f9"}}, "fnd-08-data-encoding": {"tiers": ["T0", "T0", "T0"], "learn": ["Base64 turns bytes into a safe-to-transport alphabet — reversible by anyone. You’ll decode it constantly.", "Hex is base-16 bytes; reading it on sight is table stakes for analysis.", "The trap: encoding ≠ encryption. Base64’d secrets are not protected — that needs hashing or a key."], "links": [{"href": "https://developer.mozilla.org/en-US/docs/Glossary/Base64", "label": "MDN — Base64"}, null, null], "realenv": {"t": "Build a decoder", "learn": "Chain base64→gzip→json over a real sample in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{decoded}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{decoded}</code>", "flagPrefix": "27bca09f54bff69b"}}, "fnd-09-cryptography": {"tiers": ["T0", "T0", "T0"], "learn": ["A hash is deterministic, one-way, fixed-length; one flipped input bit flips ~half the output (avalanche).", "Asymmetric crypto uses a keypair: encrypt with the public key, decrypt with the private — no shared secret needed.", "The flag checks you keep meeting are just hashes: we store the digest, you prove the input. Zero answer-leak."], "links": [{"href": "https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto", "label": "MDN — SubtleCrypto"}, null, null], "realenv": {"t": "Sign with OpenSSL", "learn": "Generate a key and sign/verify a file with real openssl in a Codespace.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{hashed}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{hashed}</code>", "flagPrefix": "c5545e7d02a10945"}}, "fnd-10-scripting": {"tiers": ["T0", "T0", "T0"], "learn": ["Security scripting is mostly extract → filter → report. Regex pulls IOCs (IPs, hashes, domains) out of noise.", "Exit codes are how a script talks to a pipeline: 0 passes the gate, non-zero fails it — set them deliberately.", "Filtering to the signal (just the ERRORs) is the first script you’ll reuse forever. AI drafts it; you review every line."], "links": [{"href": "https://docs.python.org/3/library/re.html", "label": "Python — re module"}, null, null], "realenv": {"t": "Run real Python", "learn": "Run the parser over an nmap CSV in a Codespace and emit a Markdown report.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{script-ran}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{script-ran}</code>", "flagPrefix": "cbc15e2f33e43823"}}, "fnd-11-version-control": {"tiers": ["T0", "T0", "T0"], "learn": ["Commit is local; push publishes. The gap is where ‘I lost my work’ and ‘I pushed a secret’ both live.", "A committed secret is public forever once pushed — pre-commit/CI secret scanning and a good .gitignore exist for exactly this.", "Force-push rewrites shared history and can erase others’ commits; rewrite only your own un-pushed work."], "links": [{"href": "https://git-scm.com/book/en/v2", "label": "Pro Git (book)"}, null, null], "realenv": {"t": "Real git + gitleaks", "learn": "Set up a repo with a pre-commit secret scan in a Codespace and make a clean signed commit.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{committed}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{committed}</code>", "flagPrefix": "0f90c96309774f9d"}}, "fnd-12-threat-modeling": {"tiers": ["T0", "T0", "T0"], "learn": ["STRIDE names the threat classes: Spoofing, Tampering, Repudiation, Info-disclosure, DoS, Elevation. Forging an identity is Spoofing.", "The method is an order: diagram the system → enumerate threats per element → mitigate by risk. Skipping the diagram skips the threats.", "Trust boundaries — where data crosses from less- to more-trusted — are where controls belong and where most findings live."], "links": [{"href": "https://owasp.org/www-community/Threat_Modeling_Process", "label": "OWASP — Threat Modeling Process"}, null, null], "realenv": {"t": "Build the model", "learn": "Draft a DFD + STRIDE table for a small app in a Codespace and commit it.", "href": "https://codespaces.new/plaintext-security/plaintext-labs?quickstart=1", "launchLabel": "Launch in GitHub Codespaces", "afterLaunch": "Then run the lab steps and paste the flag your run prints (same hash-check the in-browser bites use):", "placeholder": "FLAG{model-built}", "okMsg": "Verified — the real-container → flag loop, identical to <code>make check</code>.", "noMsg": "In the real lab <code>make check</code> prints this. For the POC, paste: <code>FLAG{model-built}</code>", "flagPrefix": "24158ac75730c388"}}};
