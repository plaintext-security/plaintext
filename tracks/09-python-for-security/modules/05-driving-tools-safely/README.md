...
- **Many sensors, one truth** (stretch) — the same incident, three schemas: Suricata **EVE JSON**
  (opinionated alerts), Zeek native **TSV** (`#fields` header, descriptive facts), and Sysmon **EVTX**
  (binary → XML `EventData`, the *host* side — which process opened a connection). A source-agnostic domain
  model normalizes all three — the real payoff of "parse, don't trust," and the join from a network IOC to
