...
!!! abstract "In 60 seconds"
    A cryptographic primitive makes *one* guarantee and nothing more — AES gives confidentiality,
    a MAC gives integrity, AES-GCM gives both. Choosing the wrong one for your threat model is a
    design flaw, not a tuning issue. The Adobe 2013 breach (153M accounts, 3DES-ECB, unsalted)
    proves it: they reached for *encryption* where a *salted password hash* was the tool, in a
    *mode* that leaked structure. Mode matters more than key size; correct implementation matters
    more than both.
...
!!! note "The mental model"
    A primitive is a building block, not a system. Before reaching for an API, name the guarantee
    you need — confidentiality, integrity, authenticity, or some combination — then pick the
    primitive that provides exactly that. "Choose strong crypto" is not guidance; "I need authenticated
    encryption because the channel is hostile, so AES-GCM" is.

!!! warning "The gotcha"
    Confidentiality is not integrity. A plain cipher (AES-CBC, AES-CTR) hides content but does not
    detect tampering — an attacker can flip ciphertext bits and decryption succeeds, producing
    corrupted plaintext you may never notice. If you encrypted but didn't authenticate, you only
    bought half of what you assumed.
...
??? note "Go deeper: why AES-256 over AES-128 isn't the upgrade you think"
    Doubling the key size feels like the obvious "more secure" knob, but for classical adversaries
    AES-128's 128-bit security is already beyond brute-force. AES-256's edge is purely against
    Grover's quantum search (which roughly halves effective key length). Meanwhile an ECB-mode
    AES-256 deployment leaks structure that AES-128-GCM would not — so the knob you reached for
    bought nothing while the one you ignored was the actual flaw.

!!! tip "AI caveat"
    An AI explains why CBC is vulnerable to a padding oracle fluently and usually correctly — but it
    will also state subtly wrong claims with the same confidence. Cross-check any technical
    assertion against a primary source (the Vaudenay paper, OWASP) before you treat it as fact.
...
!!! question "Check yourself"
    - You need to store a value so an attacker who reads the database can't recover it *and* can't silently alter it. Which guarantee(s) do you need, and which primitive provides them?
    - Why did Adobe's choice of ECB mode let researchers recover the most common passwords without ever finding the key?
    - Rank these by how much they affect real security: key size (128 vs 256), mode choice (ECB vs GCM), implementation correctness. Justify the order.
