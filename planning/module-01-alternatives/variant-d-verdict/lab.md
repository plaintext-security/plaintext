...
(call it Meridian) has just realized their account is shaped exactly like Capital One's was: a server
...
1. [ ] **Hop 2 — metadata handed out credentials.**
   ...
   **Do:** inspect the EC2 instance role and its trust
   (`awslocal iam get-role --role-name MeridianEC2InstanceRole`). Note that nothing scopes how powerful
...
2. [ ] **Hop 3 — the role could read *every* bucket.**
   ...
   **Do:** this is the heart of the breach — reproduce it. Confirm the role's policy
   (`MeridianDevPolicy`) grants `s3:*` on `*`, then prove the blast radius with
   `awslocal iam simulate-principal-policy`: ask whether the role can `s3:GetObject` on
   `meridian-uploads-dev` **and** on a second, unrelated bucket you create
   (`awslocal s3api create-bucket --bucket meridian-payroll-prod`). Both come back `allowed`.
...
scoped version. Run it against both the broken `MeridianDevPolicy` and your fixed policy and show it
