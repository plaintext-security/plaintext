...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form, build-first. The skill here is procedural —
write least-priv RBAC and NetworkPolicy as code — so the module leads with the build and lets a real
breach carry the stakes, not a contrived quiz. One *light* prediction (the compromised-pod blast radius)
because the default answer is genuinely alarming and load-bearing. See ../DESIGN.md row 12. -->
