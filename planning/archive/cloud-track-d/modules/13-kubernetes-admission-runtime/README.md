...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D, but build-first per DESIGN.md row 13 — the skill
*is* "write the Kyverno policy," so the build leads and the real breach (Graboid) carries the stakes.
There is no front-loaded "core idea" essay; a light predict prompt ("which pod specs should never have
been admitted?") seeds the bridge, then the lab is the teaching. Do NOT escalate the predict into a
contrived quiz — the load-bearing learning is the policy and the Falco rule. See ../DESIGN.md row 13. -->
{ .module-meta }
