...
<!-- AUTHOR'S NOTE (delete before publish): This is a build-first module, not predict-then-reveal.
The real CVE and the shared-kernel mechanism carry the stakes; the learner reproduces an actual
exploit and then encodes the detection. There is one light prediction (which syscall betrays the
escape) because the naive answer — "the mount" — is instructively incomplete. See DESIGN.md row 11. -->
