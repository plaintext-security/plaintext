...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form, predict-what-fires/reconstruct shape (DESIGN row 16). This is the final module before the capstone — it should feel like the payoff. No front-loaded essay; the bridge arrives as a prediction the learner commits to (why did "contained" fail?) then the reveal, then a hands-on reconstruction. The real tool/technical content (CloudTrail timeline, IOCs, containment order, hayabusa methodology, extending triage.py) is preserved from the shipped module — only the frame changed. See ../DESIGN.md row 16. -->
