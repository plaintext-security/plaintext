...
<!-- AUTHOR'S NOTE (delete before publish): This is the track's judgment-as-code EXEMPLAR. Per
DESIGN.md row 06 the shape is build-first, not predict-then-reveal — the deliverable IS a CI gate.
The single prediction prompt is deliberately light and instructive ("which lines fail, which get
missed?") and exists to set up the module's real lesson: a scanner catches the known-bad *pattern*
but not the bad *decision*, and the gate is where you encode the decision so it can't regress.
Every downstream build module and the capstone reuse this gate. See ../DESIGN.md. -->
