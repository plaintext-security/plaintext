...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form. The bridge arrives as a "predict the
injection point" call the learner commits to before the reveal, then a build-first lab that ends in the
hardened, provenance-signed workflow as judgment-as-code. This is the build/pipeline exemplar — the real
breach (SolarWinds) carries the stakes; the deliverable is the hardened workflow. See ../DESIGN.md row 08. -->
