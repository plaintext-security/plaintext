...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form, but this is a PURPLE-TEAM module — the
"detonate" shape, not predict-then-reveal-a-verdict. There is no build/fix half: this module's job is
to generate real adversary signal in a safe range and *hand the telemetry to module 15 (detect) and 16
(respond)*. Keeping it pure is deliberate per DESIGN.md row 14. The "Automate & own it" step is a safe,
repeatable detonation harness — the right automation for an attack module, not a guardrail. -->
