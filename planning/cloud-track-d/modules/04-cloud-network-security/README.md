...
bridge arrives as a reachability prediction the learner commits to before the reveal, then as an
audit→build→re-verify lab that ends in a default-deny baseline proven by a reachability check and a
Checkov rule. Specialist module, so Learn curates more. See ../DESIGN.md row 04. -->
