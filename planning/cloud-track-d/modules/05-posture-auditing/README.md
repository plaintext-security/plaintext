...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form, but this is an *audit→remediate→verify*
module (DESIGN.md row 05), not a predict-then-reveal one — intuition doesn't mislead here, the skill
is triage and verification. So the bridge leads with the real anchor and the linter mental model, and
the one light prediction is "how many findings, and which one matters." Preserves the shipped module's
real content (prowler/scoutsuite, CIS, signal-vs-noise triage, remediate+verify); changes the frame to
the 2017 S3 leak wave and adds the verified-benchmark guardrail. See ../DESIGN.md row 05. -->
