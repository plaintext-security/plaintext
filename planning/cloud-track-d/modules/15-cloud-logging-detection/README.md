...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form. The bridge is delivered as a predict-what-fires
pair the learner commits to before the reveal, then re-walked in the lab against module-14 telemetry. This
module pairs directly with 14 — you detect the techniques you detonated there. Preserves the shipped
module's real content (management vs. data events, Sigma, native detectors, sequence logic, FP tuning),
changes the frame from "here's the logging surface" to "the log existed; predict the gap." See ../DESIGN.md row 15. -->
