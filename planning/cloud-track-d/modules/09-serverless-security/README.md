...
bridge arrives as a blast-radius prediction the learner commits to before the reveal, then an
attacker→fixer lab that ends in the role least-privileged and the fix proven as a guardrail. Phase-2
opener, so it leans on the IAM model from 02/03 rather than re-deriving it. See ../DESIGN.md row 09. -->
