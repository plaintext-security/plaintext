...
<!-- AUTHOR'S NOTE (delete before publish): Variant-D form, build-first row. This is a procedural
operate module, so per DESIGN.md the breach carries the stakes and the *build* is the spine — there's
no front-loaded blast-radius quiz, just one light predict prompt (the post-breach rotation that didn't
contain anything) at the point where the static-vs-leased distinction lands. The shipped module's
operate side was shallow (store/read/rotate a KV secret); this deepens it to a leased dynamic credential
the app fetches at runtime, automated rotation, and an IAM-gated native store. See DESIGN.md row 07. -->
...
- [GitGuardian — State of Secrets Sprawl (latest annual report)](https://www.gitguardian.com/state-of-secrets-sprawl-report-2024) (~15 min, skim) — the scale: millions of secrets leaked to public commits per year. Skim the headline numbers; it's the "this is common, not exotic" evidence. <!-- VALIDATE current-year URL -->
...
- [Vault — dynamic secrets / leases tutorial](https://developer.hashicorp.com/vault/tutorials/get-started/dynamic-secrets) (~45 min) — work through it hands-on: lease TTL, renew, revoke. The TTL is the whole point — it's why a leaked dynamic credential is harmless. <!-- VALIDATE exact tutorial slug -->
...
- [AWS — IAM policy for Secrets Manager (`Resource`-scoped read)](https://docs.aws.amazon.com/secretsmanager/latest/userguide/auth-and-access_identity-based-policies.html) (~15 min) — how you gate a native store so *only* the app role can read *one* secret ARN — the IAM guardrail half. <!-- VALIDATE -->
