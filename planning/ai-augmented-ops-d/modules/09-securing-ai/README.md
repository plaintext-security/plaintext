...
- [Aim Security — *EchoLeak (CVE-2025-32711): the first zero-click attack on an AI agent*](https://www.aim.security/lp/aim-labs-echoleak-blogpost) — the discovering researchers' writeup of the M365-Copilot chain: untrusted email → "LLM Scope Violation" → data exfil through an allowed image fetch, with no user interaction. Read it for the *mechanism* (how a system prompt was structurally unable to help), not the marketing.<!-- VALIDATE: confirm Aim Security blog URL resolves; fallback primary = MSRC advisory below + The Hacker News writeup -->
...
- The Chevrolet of Watsonville "\$1 Tahoe" bot (Chris Bakke, Nov 2023) — the canonical "just tell it to agree" jailbreak. The original is a viral X screenshot; cite a durable secondary like [Business Insider's account](https://www.businessinsider.com/car-dealership-chevrolet-chatbot-chatgpt-pranks-chevy-2023-12) rather than the ephemeral post.<!-- VALIDATE: confirm a durable secondary source for the Chevy bot; the original X post may rot -->
